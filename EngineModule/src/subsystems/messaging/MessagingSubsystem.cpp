...
MessagingSubsystem::MessagingSubsystem()
    : m_messagesProcessedPerFrame(100) {}

bool MessagingSubsystem::Initialize() {
    std::cout << "Initializing Messaging Subsystem..." << std::endl;
    std::cout << "Messages per frame: " << m_messagesProcessedPerFrame << std::endl;
    return true;
}
...
void MessagingSubsystem::Update(float deltaTime) {
    ProcessMessages();
}
...
void MessagingSubsystem::SendSyncMessage(const std::string& type, std::any data) {
    DispatchMessage(Message(type, std::move(data), MessagePriority::CRITICAL, true));
}

void MessagingSubsystem::Subscribe(const std::string& messageType, MessageCallback callback) {
