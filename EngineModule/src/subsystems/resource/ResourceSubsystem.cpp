...
bool ResourceSubsystem::Initialize() {
    ...
    std::cout << "Cache limit: " << (m_cacheLimit / 1024 / 1024) << " MB" << std::endl;
    return true;
}
...
void ResourceSubsystem::RegisterResource(const std::string& id, ResourceType type,
                                        const std::string& filepath, LoadStrategy strategy) {
    ...
    std::cout << "Registered resource: " << id << " (strategy: " 
              << static_cast<int>(strategy) << ")" << std::endl;
}

std::shared_ptr<ResourceHandle> ResourceSubsystem::LoadResource(const std::string& id) {
    ...
    if (LoadResourceImpl(id, *handle)) {
        ...
        std::cout << "Loaded resource: " << id 
                 << " (" << handle->memorySize << " bytes)" << std::endl;
        return handle;
    }
...
void ResourceSubsystem::UnloadUnusedResources() {
    ...
    std::cout << "Unloaded " << toRemove.size() << " unused resources" << std::endl;
}

std::shared_ptr<ResourceHandle> ResourceSubsystem::GetResource(const std::string& id) {
    auto it = m_resources.find(id);
    if (it != m_resources.end()) {
        return it->second;
    }
...
void ResourceSubsystem::StartPreload() {
    ...
    m_isPreloading = true;
    m_preloadProgress = 0.0f;
    std::cout << "Starting preload of " << m_preloadQueue.size() << " resources" << std::endl;
}
...
bool ResourceSubsystem::LoadResourceImpl(const std::string& id, ResourceHandle& handle) {
