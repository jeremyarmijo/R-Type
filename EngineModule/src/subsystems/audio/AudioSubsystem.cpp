...
AudioSubsystem::~AudioSubsystem() { Shutdown(); }
...
bool AudioSubsystem::LoadMusic(const std::string& name,
  ...
  if (!music) {
    std::cerr << "Failed to load music: " << filepath << " - " << Mix_GetError()
              << std::endl;
    return false;
  }
...
void AudioSubsystem::PauseMusic() { Mix_PauseMusic(); }
void AudioSubsystem::ResumeMusic() {
  if (!m_musicMuted) {
    Mix_ResumeMusic();
  }
}
bool AudioSubsystem::LoadSound(const std::string& name,
                               const std::string& filepath) {
...
void AudioSubsystem::PlaySound(const std::string& name, int loops,
                               int channel) {
...
void AudioSubsystem::StopSound(int channel) { Mix_HaltChannel(channel); }
...
extern "C" {
ISubsystem* CreateSubsystem() { return new AudioSubsystem(); }
}
