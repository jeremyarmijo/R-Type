...
void UIButton::Render(SDL_Renderer* renderer, RenderingSubsystem* renderSys) {
  if (!m_visible) return;
  ...
  if (!m_textureKey.empty() && renderSys) {
    SDL_Texture* texture = renderSys->GetTexture(m_textureKey);
    if (texture) {
      if (!m_enabled) {
