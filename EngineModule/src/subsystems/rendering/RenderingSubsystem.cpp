...
bool RenderingSubsystem::Initialize() {
    ...
    m_uiManager = std::make_unique<UIManager>(m_renderer, this, m_windowWidth, m_windowHeight);
...
void RenderingSubsystem::RenderSprites() {
    ...
    for (const auto& data : renderList) {
        ...
        SDL_Rect destRect = {
            static_cast<int>(data.transform->position.x - m_cameraPosition.x -
                           (sourceRect.w * data.transform->scale.x * data.sprite->pivot.x)),
            static_cast<int>(data.transform->position.y - m_cameraPosition.y -
...
bool RenderingSubsystem::LoadTexture(const std::string& key, const std::string& filepath) {
...
void RenderingSubsystem::GetTextureSize(const std::string& key, int& width, int& height) {
...
void RenderingSubsystem::CreateAnimation(const std::string& name, const std::string& textureKey,
                                        const std::vector<AnimationFrame>& frames, bool loop) {
    AnimationClip clip(loop);
    clip.textureKey = textureKey;
    clip.frames = frames;
    m_animations[name] = clip;
    std::cout << "Created animation: " << name << " (" << frames.size() << " frames)" << std::endl;
}

const AnimationClip* RenderingSubsystem::GetAnimation(const std::string& name) const {
