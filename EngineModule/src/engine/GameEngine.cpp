...
bool GameEngine::LoadSubsystem(SubsystemType type,
                               const std::string& pluginPath) {
  if (HasSubsystem(type)) {
    std::cerr << "Subsystem already loaded: " << static_cast<int>(type)
              << std::endl;
    return false;
  }
  ...
  try {
    ...
    auto loader =
        std::make_unique<DLLoader<ISubsystem>>(pluginPath, "CreateSubsystem");
    ...
    std::cout << "Created subsystem: " << subsystem->GetName() << " v"
              << subsystem->GetVersion() << std::endl;
    ...
    std::cout << "Subsystem loaded successfully: " << subsystem->GetName()
              << std::endl;
    return true;
...
void GameEngine::UnloadSubsystem(SubsystemType type) {
  ...
  std::cout << "Unloading subsystem: " << it->second.instance->GetName()
            << std::endl;
