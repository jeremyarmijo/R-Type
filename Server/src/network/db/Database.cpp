...
bool Database::PrepareStatement(const std::string &query, void **stmt) {
  return sqlite3_prepare_v2(db_, query.c_str(), -1,
                            reinterpret_cast<sqlite3_stmt **>(stmt),
                            nullptr) == SQLITE_OK;
}
...
bool Database::GetUser(const std::string &username, std::string &password,
                       int &score) {
  sqlite3_stmt *stmt;
  std::string query = "SELECT password, score FROM users WHERE username = ?";
  if (!PrepareStatement(query, reinterpret_cast<void **>(&stmt))) return false;
...
bool Database::AddUser(const std::string &username, const std::string &password,
  ...
  if (!PrepareStatement(query, reinterpret_cast<void **>(&stmt))) {
    std::cerr << "Failed to prepare statement for adding user: " << username
              << std::endl;
    return false;
  }
  sqlite3_bind_text(stmt, 1, username.c_str(), -1, SQLITE_STATIC);
