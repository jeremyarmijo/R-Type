...
UDPServer::UDPServer(asio::io_context& io_context, uint16_t port)
    ...
    std::cout << "[UDPServer] Listening on port " << port << std::endl;
    std::cout << "[UDPServer] Socket opened: " << socket_.local_endpoint().address().to_string() << std::endl;
    StartReceive();
}
...
void UDPServer::SendTo(const std::vector<uint8_t>& data, 
    ...
    socket_.async_send_to(
        ...
        [](const asio::error_code& error, std::size_t bytes) {
            if (error) {
                std::cerr << "[UDPServer] Send error: " << error.message() << std::endl;
            }
        }
    );
}
