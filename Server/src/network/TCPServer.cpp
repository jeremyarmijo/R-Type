        ...
        [this](const asio::error_code& error, asio::ip::tcp::socket socket) {
            if (!error) {
                ...
                auto session = std::make_shared<TCPSession>(
                    std::move(socket), client_id, this);
...
TCPSession::TCPSession(asio::ip::tcp::socket socket, uint32_t client_id, TCPServer* server)
    : socket_(std::move(socket)), 
      endpoint_(socket_.remote_endpoint()),
      client_id_(client_id),
      server_(server) {
    std::cout << "[TCPSession] New connection from " 
              << endpoint_.address().to_string() << ":" << endpoint_.port()
              << " (Client ID: " << client_id_ << ")" << std::endl;
}

TCPSession::~TCPSession() {
    std::cout << "[TCPSession] Disconnected client " << client_id_ << std::endl;
}
...
void TCPSession::HandleReadHeader(const asio::error_code& error, std::size_t bytes_transferred) {
    if (error) {
        if (error != asio::error::eof && error != asio::error::operation_aborted) {
            std::cerr << "[TCPSession] Header read error: " << error.message() << std::endl;
        }
    ...
    std::cout << "[TCPSession] Received header: type=0x" << std::hex << (int)type 
              << " flags=0x" << (int)flags << " length=" << std::dec << length << std::endl;
    
    // Vérifier taille raisonnable
    if (length > 1024) {
        std::cerr << "[TCPSession] Payload too large: " << length << std::endl;
        Close();
        return;
    }
...
void TCPSession::ReadPayload(uint16_t payload_size) {
...
void TCPSession::HandleReadPayload(const asio::error_code& error, std::size_t bytes_transferred) {
    if (error) {
        std::cerr << "[TCPSession] Payload read error: " << error.message() << std::endl;
        return;
    }
    ...
    switch (current_msg_type_) {
        ...
        default:
            std::cerr << "[TCPSession] Unknown message type: 0x" 
                      << std::hex << (int)current_msg_type_ << std::endl;
            break;
    }
...
void TCPSession::ProcessLoginRequest() {
    ...
    username_ = std::string(
        reinterpret_cast<char*>(&payload_buffer_[offset]),
        username_len
    );
    offset += username_len;
    ...
    std::cout << "[TCPSession] Login request from '" << username_ 
...
void TCPSession::SendLoginResponse(bool success, uint32_t player_id, uint16_t udp_port) {
    ...
    asio::async_write(socket_,
        ...
        [this, self](const asio::error_code& error, std::size_t bytes) {
            if (error) {
                std::cerr << "[TCPSession] Send error: " << error.message() << std::endl;
            } else {
                std::cout << "[TCPSession] LOGIN_RESPONSE sent to client " 
                          << client_id_ << std::endl;
            }
        }
    );
...
void TCPSession::SendLoginError(uint16_t error_code, const std::string& message) {
    std::vector<uint8_t> response;
    ...
    asio::async_write(socket_,
        ...
        [this, self](const asio::error_code& error, std::size_t bytes) {
            if (!error) {
                std::cout << "[TCPSession] LOGIN_ERROR sent to client " 
                          << client_id_ << std::endl;
            }
