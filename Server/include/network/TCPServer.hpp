 ...
 * 
 * Manages TCP connections, client authentication, and reliable message delivery.
 */
class TCPServer {
 public:
  using LoginCallback =
  ...
  using DisconnectCallback = std::function<void(uint16_t client_id)>; ///< Disconnect callback type
 ...
 private:
  ...
  std::unordered_map<uint16_t, std::shared_ptr<ProcessPacketTCP>> sessions_;  ///< Active client sessions
...
class ProcessPacketTCP : public std::enable_shared_from_this<ProcessPacketTCP> {
 ...
 private:
  ...
  std::array<uint8_t, HEADER_SIZE> header_buffer_;  ///< Buffer for packet header
