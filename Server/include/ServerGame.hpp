...
class ServerGame {
 ...
 private:
  ...
  std::queue<std::tuple<Event, uint16_t>> eventQueue;   ///< Queue of incoming events from clients
  std::queue<std::tuple<Action, uint16_t>> actionQueue; ///< Queue of actions to send to clients
