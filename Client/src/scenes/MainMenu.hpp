 ...
 public:
  ...
  void OnEnter() override {
    ...
    try {
      ...
      RenderingSubsystem* rendering = GetRendering();
      AudioSubsystem* audio = GetAudio();

      audio->LoadMusic("menu_music", "../assets/menu_music.ogg");
      audio->LoadMusic("game_music", "../assets/rtype_music.ogg");
      audio->LoadSound("explosion", "../assets/explosion.wav");
      audio->LoadSound("button", "../assets/button.wav");
      audio->LoadSound("shoot", "../assets/shoot.wav");
      audio->LoadSound("hitmarker", "../assets/hitmarker.wav");

      audio->PlayMusic("menu_music");

      std::cout << "Loading textures..." << std::endl;
      if (!rendering->GetTexture("player")) {
        rendering->LoadTexture("player", "../assets/player.png");
      }
      if (!rendering->GetTexture("boss")) {
        rendering->LoadTexture("boss", "../assets/boss1.png");
      }
      if (!rendering->GetTexture("background")) {
        rendering->LoadTexture("background", "../assets/bg.jpg");
      }

      std::cout << "Creating animations..." << std::endl;
      rendering->CreateAnimation("boss", "boss",
                                 {{{27, 2, 154, 203}, 0.6f},
                                  {{189, 2, 154, 203}, 0.5f},
                                  {{351, 2, 154, 203}, 0.6f},
                                  {{189, 2, 154, 203}, 0.5f}},
                                 true);
      rendering->CreateAnimation("bosschild", "boss",
                                 {{{570, 1964, 31, 31}, 0.5f},
                                  {{603, 1964, 31, 31}, 0.6f},
                                  {{636, 1964, 31, 31}, 0.5f},
                                  {{603, 1964, 31, 31}, 0.6f}},
