...
class GameOverScene : public Scene {
 private:
  ...
  bool m_isVictory;
  std::vector<std::tuple<uint16_t, std::string, uint32_t, uint8_t>>
 ...
 public:
  ...
  void OnEnter() override {
    ...
    m_scores = GetSceneData()
                   .Get<std::vector<std::tuple<uint16_t, std::string, uint32_t, uint8_t>>>(
                       "scores", {});
    ...
    int yPos = 200;
    for (const auto& [playerId, name, score, rank] : m_scores) {
      std::string rankStr = "#" + std::to_string(rank);
      std::string playerStr = " : " + name;
    ...
    for (const auto& [playerId, name, score, rank] : m_scores) {
      std::cout << "  Rank " << static_cast<int>(rank) << ": "
                << name << " - " << score << " pts" << std::endl;
    }
  }
