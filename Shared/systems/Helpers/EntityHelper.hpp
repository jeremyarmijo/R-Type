...
inline Entity createEnemy(Registry& registry, EnemyType type,
                          ...
                          float hpMultiplier = 1.0f, uint8_t diff = 1) {
  ...
  switch (diff) {
    case 1: mHP = 0.5f; mSpd = 0.7f; mDmg = 0.3f; mScr = 0.5f; break;
    case 2: mHP = 1.0f; mSpd = 0.9f; mDmg = 0.7f; mScr = 1.0f; break;
    case 3: mHP = 1.4f; mSpd = 1.0f; mDmg = 1.1f; mScr = 2.0f; break;
    case 4: mHP = 2.0f; mSpd = 1.1f; mDmg = 1.5f; mScr = 4.0f; break;
    case 5: mHP = 3.0f; mSpd = 1.3f; mDmg = 2.0f; mScr = 8.0f; break;
    default: break;
  }
  float finalHp = (baseHp * mHP);
  float finalSpeed = (baseSpeed * mSpd);
  int finalDamage = static_cast<int>(damage * mDmg);
  int finalScore = static_cast<int>(score * mScr);
  ...
  registry.add_component<Enemy>(
      enemy, Enemy{type, finalSpeed, {-1, 0}, 80.f, finalHp, finalScore, 0, finalDamage});
