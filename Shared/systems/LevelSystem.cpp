...
std::vector<LevelComponent> createLevels() {
  ...
  levels.emplace_back(std::vector<Wave>{
    ...
    levels.emplace_back(std::vector<Wave>{
      ...
      Wave{{EnemyType::Zigzag, EnemyType::Chase},
           {2, 3},
           {{700, 110}, {700, 170}, {700, 230}, {700, 390}, {700, 450}},
           false,
           std::nullopt,
           0},
      Wave{{}, {0}, {{700, 250}}, true, BossType::Gomander_snake, 1200}});
  ...
  levels.emplace_back(std::vector<Wave>{
      ...
      Wave{{EnemyType::Zigzag, EnemyType::Chase},
           ...
           std::nullopt,
           0},
      Wave{{}, {0}, {{700, 300}}, true, BossType::Bydo_Battleship, 3600}});

  return levels;
}

bool update_level_system(Registry& registry, float deltaTime, int levelIndex, uint8_t diff) {
  ...
  if (wave.isBossWave) {
    ...
    if (btype == BossType::Gomander_snake) {
      ...
      std::vector<Vector2> offsets = {
           {-250.f, -80.f},  // Coin avant-haut (avant-gauche visuel)
           {-250.f, 80.f},   // Coin avant-bas (avant-droit visuel)
          {250.f, -80.f},   // Coin arrière-haut (arrière-gauche visuel)
           {250.f, 80.f},    // Coin arrière-bas (arrière-droit visuel)
  };
