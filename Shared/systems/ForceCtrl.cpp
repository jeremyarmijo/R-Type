  ...
  for (auto&& [forceIdx, forceTransform, forceCollider, force] :
      ...
      if (check_collision(forceTransform, forceCollider, enemyTransform,
        ...
        if (enemy.current <= 0) {
          if (playerId < players.size() && players[playerId].has_value()) {
            players[playerId]->score += enemy.scoreValue;
            std::cout << "[SCORE] Player " << players[playerId]->player_id
                      << " gained " << enemy.scoreValue
                      << " points (Force kill)! Total: " << players[playerId]->score
                      << std::endl;
          }
    ...
    for (auto&& [bossIdx, bossTransform, bossCollider, boss] :
      ...
      if (check_collision(forceTransform, forceCollider, bossTransform,
        ...
        if (boss.current <= 0) {
          if (playerId < players.size() && players[playerId].has_value()) {
            players[playerId]->score += boss.scoreValue;
            std::cout << "[SCORE] Player " << players[playerId]->player_id
                      << " killed BOSS with Force! +" << boss.scoreValue
                      << " points! Total: " << players[playerId]->score
                      << std::endl;
          }
