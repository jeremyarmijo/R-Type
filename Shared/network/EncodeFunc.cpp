...
void GameEndFunc(const Action& a, std::vector<uint8_t>& out) {
  ...
  out.clear();
  out.push_back(end->victory ? 1 : 0);
  out.push_back(static_cast<uint8_t>(end->scores.size()));

  for (const auto& s : end->scores) {
    uint16_t playerId = htons(s.playerId);
    uint8_t idBuf[2];
    memcpy(idBuf, &playerId, 2);
    out.push_back(idBuf[0]);
    out.push_back(idBuf[1]);

    uint8_t nameLen = static_cast<uint8_t>(s.playerName.size());
    out.push_back(nameLen);
    out.insert(out.end(), s.playerName.begin(), s.playerName.end());

    uint32_t scoreNet = htonl(s.score);
    uint8_t scoreBuf[4];
    memcpy(scoreBuf, &scoreNet, 4);
    for(int i = 0; i < 4; ++i) out.push_back(scoreBuf[i]);

    out.push_back(s.rank);
  }
}
