...
struct Weapon {
    ...
    explicit Weapon(float rate = 1.0f, bool automatic = false,
                    int ammo = -1, int magSize = -1,
                    float reload = -1.0f, bool burst = false,
                    size_t bCount = 3, float bInterval = 0.1f)
        : fireRate(rate), isAutomatic(automatic), maxAmmo(ammo),
        magazineSize(magSize), reloadTime(reload), isBurst(burst),
        burstCount(bCount), burstInterval(bInterval), timeSinceLastShot(0.0f),
        currentAmmo(magSize) {}
};
